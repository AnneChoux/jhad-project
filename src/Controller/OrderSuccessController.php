...
class OrderSuccessController extends AbstractController
{
    ...
    public function index(CartServices $cartServices, $stripeSessionId)
    {
        ...
        #Si le statut de la commande est 0
        if($order->getStatus() == 0){

            #v #On remove le panier(on a besion d'importer la class Cartservice )
            $cartServices->remove();

            #On change la statut de la commande au statut suivant=>on envoie dans la base de données par la suite passant le statut à 1
            $order->setStatus(1);
